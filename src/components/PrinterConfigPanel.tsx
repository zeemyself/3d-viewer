...
export function PrinterConfigPanel({ model }: PrinterConfigPanelProps) {
  ...
  const basicSettings = [
    ...
    { label: "Layer Height", value: config.layerHeight ? `${config.layerHeight} mm` : undefined },
    { label: "Infill Density", value: config.infill ? `${config.infill}%` : undefined },
    { label: "Nozzle Temp", value: config.printTemp ? `${config.printTemp}C` : undefined },
    { label: "Bed Temp", value: config.bedTemp ? `${config.bedTemp}C` : undefined },
    { label: "Support", value: config.supportEnabled ? formatValue(config.supportType) || "Enabled" : "Disabled" },
  ...
  const allSettings = config.allSettings
    ? Object.entries(config.allSettings)
        .filter(([key]) => !basicSettings.some((s) => s.label.toLowerCase().includes(key.toLowerCase())))
      ...
      <div className="panel-scroller p-4">
        ...
        {allSettings.length > 0 && (
          <div className="mt-3.5">
            ...
            >
              ...
              <span className="flex items-center gap-2">
                <span
                  ...
                  style={{ background: "rgba(119, 79, 159, 0.14)", fontFamily: "var(--font-mono)" }}
                >
                  {allSettings.length}
                </span>
                <svg
            ...
            {showAll && (
              <div className="panel-surface mt-2 space-y-1.5 p-2.5" style={{ borderRadius: "0.8rem" }}>
                {allSettings.map(([key, value]) => (
                  ...
                  >
                    <span className="max-w-[52%] truncate text-xs" style={{ color: "var(--text-tertiary)" }}>
                      {key}
                    </span>
                    <span
